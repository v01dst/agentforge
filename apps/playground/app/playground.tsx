  ...
  async function runAgent(event?: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/runs", {
        ...
        body: JSON.stringify({ workflow: "Agent console", input: { topic: input }, provider, model }),
      });
  ...
  return (
    <div className="console-shell">
      <aside className="console-sidebar">
        ...
        <button className="side-link"><FileCode2 size={16} /><span>Workflows</span></button>
        ...
        <button className="side-link"><KeyRound size={16} /><span>Environment</span></button>
      ...
      {showSettings ? <div className="modal-backdrop" onClick={() => setShowSettings(false)}><section className="settings-modal" onClick={(event) => event.stopPropagation()}><div className="modal-header"><div><p className="eyebrow">Runtime settings</p><h2>Connect a model</h2></div><button className="header-icon" onClick={() => setShowSettings(false)}><X size={16} /></button></div><label>Provider<select value={provider} onChange={(event) => setProvider(event.target.value)}><option value="mock">Mock (offline)</option><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="google">Google Gemini</option><option value="custom">Custom provider</option></select></label><label>Model<input value={model} onChange={(event) => setModel(event.target.value)} placeholder="gpt-4o-mini" /></label><div className="settings-note"><KeyRound size={14} /><span>Credentials stay in your environment. The playground never stores API keys.</span></div><button className="save-settings" onClick={() => setShowSettings(false)}><Check size={15} />Save session settings</button></section></div> : null}
    </div>
  );
}
