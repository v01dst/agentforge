...
import { HELP, VERSION, chatCommand, connectCommand, devCommand, doctorCommand, initCommand, inspectCommand, listCommand, mcpCommand, modelsCommand, modelsTestCommand, permissionsCommand, pluginsAddCommand, pluginsCommand, pluginsLifecycleCommand, pluginsRemoveCommand, profileCommand, providersCommand, runsCommand, findingsCommand, gatewayCommand, daemonCommand, benchmarksCommand, channelsCommand, sessionsCommand, runCommand, skillsCommand, testCommand, workflowsValidateCommand } from './commands.js';
...
export async function execute(argv: string[] = process.argv.slice(2)): Promise<number> {
  ...
  switch (parsed.command) {
    ...
    case 'benchmarks': case 'benchmark': return await benchmarksCommand(parsed.args, parsed.flags);
    case 'channels': return await channelsCommand(parsed.args, parsed.flags);
