...
const files: Record<string, string> = {
  ...
  "dependencies": { "@agentforge-oss/cli": "^0.1.0", "@agentforge-oss/core": "^0.1.0", "@agentforge-oss/mcp": "^0.1.0", "@agentforge-oss/models": "^0.1.0", "zod": "^3.24.1" },
...
/** Resolution order: AGENTFORGE_PROVIDER_MODULE > managed endpoints > builtin providers (mock removed in 0.8 — configure a real provider). */
async function loadModel(): Promise<ModelProvider> {
  ...
  if (!wanted) throw new Error('No provider configured. Run "agentforge providers add <name> --base-url <url> --model <id>" (or enter an API key in the TUI), or set AGENTFORGE_PROVIDER.');
  const managed = (await readManagedProviders()).find((entry) => entry.name === wanted);
  if (managed) {
    return createModel({
      provider: (managed.protocol || 'openai-compatible') as 'openai' | 'anthropic' | 'google' | 'gemini' | 'openai-compatible',
      model: process.env.AGENTFORGE_MODEL || managed.model,
      baseUrl: managed.baseUrl,
      apiKey: managed.apiKeyEnv ? process.env[managed.apiKeyEnv] : undefined,
    });
  }
  const known = ['openai', 'anthropic', 'google', 'gemini', 'openai-compatible'];
